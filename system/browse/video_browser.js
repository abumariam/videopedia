...
ytvbp.sendRequest = function(filePath, params, resultDivName) {
  ...
  xmlhr.onreadystatechange = function() {
	if(ie)		var resultDiv=eval('document.all.'+resultDivName);
	else if(ns)	var resultDiv=eval('document.layers["'+resultDivName+'"]');
	else if(w3) var resultDiv =eval('document.getElementById("'+resultDivName+'")');
    if (xmlhr.readyState == 1) {
      resultDiv.innerHTML = "<center><img src=\"loading.gif\" border=\"0\" align=\"middle\" ><br><b> Loading...<b></center>";
...
// function to call add video php function to course "By Abdel-Kawy"
ytvbp.VIDEO_ADD_DIV = 'addVideoDiv';

ytvbp.addVideoFun = function(vidId) {
  var xvalue = document.getElementById('courseid').value;
  var params = 'queryType=add_video&videoId=' + vidId + '&crsId=' + xvalue;
  var filePath = 'index.php';
  ytvbp.sendRequest(filePath, params, ytvbp.VIDEO_ADD_DIV);
}
